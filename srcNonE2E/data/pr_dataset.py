...
def _parse_csv_line(line: tf.Tensor):
    # CSV: image_path,x1,y1,x2,y2,idx,pitch
    fields = tf.io.decode_csv(
        line,
        record_defaults=[[""], [0], [0], [0], [0], [0], [""]],
    )
    image_path, x1, y1, x2, y2, idx, pitch = fields
    return image_path, x1, y1, x2, y2, pitch


def _pitch_to_id(pitch: tf.Tensor) -> tf.Tensor:
    # Uses a static hash table (small, constant memory)
    keys = tf.constant(list(PITCH_TO_ID.keys()), dtype=tf.string)
    vals = tf.constant(list(PITCH_TO_ID.values()), dtype=tf.int32)
    table = tf.lookup.StaticHashTable(
        tf.lookup.KeyValueTensorInitializer(keys, vals),
        default_value=-1,
    )
    return table.lookup(pitch)
...
def make_pr_dataset(
    ...
    def load_crop(image_path, x1, y1, x2, y2, pitch):
        ...
        label = _pitch_to_id(pitch)
        return img, label

    ds = ds.map(load_crop, num_parallel_calls=tf.data.AUTOTUNE)

    # Drop invalid labels if any (safety)
